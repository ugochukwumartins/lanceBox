...
void main() {
  testWidgets('last invoice item can be deleted and a new item added', (
    ...
    final removeFirstItem = find.byTooltip('Remove item 1');

    // SETUP: Use in-memory preferences so this test cannot change real device data.
    ...
    await tester.tap(removeFirstItem);
    await tester.pumpAndSettle();
    // The deleted line must no longer have a remove button.
    expect(removeFirstItem, findsNothing);
    // Scroll until the target control is built and visible before interacting with it.
    await tester.scrollUntilVisible(
      find.text('Next'),
      150,
    ...
    // CHECK: The replacement item is visible.
    expect(removeFirstItem, findsOneWidget);
    // CHECK: No layout or runtime errors occurred.
